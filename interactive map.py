...
# --- NEW: ฟังก์ชันลากเส้นตามถนนและคำนวณระยะทาง (OSRM) ---
@st.cache_data
def get_road_route(points):
    """ส่งพิกัดไป OSRM เพื่อดึงเส้นตามถนนจริงและระยะทาง"""
    if len(points) < 2: return points, 0
    
    # รูปแบบ OSRM: lon,lat;lon,lat
    ...
    try:
        ...
        data = r.json()
        if data['code'] == 'Ok':
            # ดึงเส้นทางที่ดัดตามถนนแล้ว (GeoJSON ใช้ lon,lat ต้องสลับเป็น lat,lon)
        ...
        pass
    return points, 0 # ถ้า Error ให้ใช้เส้นตรงเดิม

# --- ฟังก์ชันเดิม (analyze, get_lat_lon, etc.) ---
...
def analyze_cable_issue(image_bytes):
    try:
        response = client.models.generate_content(
            ...
            contents=["""วิเคราะห์รูปภาพสายเคเบิลนี้และเลือกตอบเพียง "หนึ่งเดียว" จาก 4 สาเหตุ:
...
def create_summary_pptx(map_image_bytes, image_list):
    ...
    if image_list:
        slide2 = prs.slides.add_slide(prs.slide_layouts[6]); cols, rows = 4, 2
        ...
        for i, item in enumerate(image_list[:8]):
            ...
            txt = slide2.shapes.add_textbox(x, y + img_h + Inches(0.05), img_w, Inches(0.6)).text_frame
            p1 = txt.paragraphs[0]; p1.text = f"สาเหตุ: {item['issue']}"; p1.font.size = Pt(8); p1.font.bold = True
            p2 = txt.add_paragraph(); p2.text = f"Lat: {item['lat']:.5f}\nLong: {item['lon']:.5f}"; p2.font.size = Pt(7)
...
if kml_file:
    try:
        ...
        for pm in placemarks:
            ...
            final_name = name[0].strip() if name else "ไม่ระบุชื่อ"
...
if uploaded_files or kml_elements:
    ...
    # วาด KML (ปรับปรุง: ลากเส้นตามถนน)
    for elem in kml_elements:
        if elem['is_point']:
            loc = elem['points'][0]; all_bounds.append(loc)
            ...
            # ดึงเส้นตามถนนจริง
            with st.spinner(f"กำลังคำนวณเส้นทางถนน: {elem['name']}"):
                road_pts, dist_km = get_road_route(elem['points'])
                folium.PolyLine(road_pts, color="#FF4500", weight=6, opacity=0.8, 
                               tooltip=f"{elem['name']} (ระยะทาง: {dist_km:.2f} กม.)").add_to(m)
                all_bounds.extend(road_pts)

    # วาดรูปภาพ (เหมือนเดิม)
    if uploaded_files:
        ...
        if st.session_state.get('last_hash') != current_hash:
            ...
            for f in uploaded_files:
                f_bytes = f.getvalue(); img = ImageOps.exif_transpose(Image.open(BytesIO(f_bytes)))
    ...
    with col1: map_cap = st.file_uploader("อัปโหลดรูป Capture แผนที่", type=['jpg','png'])
