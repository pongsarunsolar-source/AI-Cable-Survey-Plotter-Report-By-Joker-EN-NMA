...
from pptx.enum.shapes import MSO_SHAPE # เพิ่มสำหรับการวาดกล่องสี่เหลี่ยม
...
# --- 7. ฟังก์ชันสร้างรายงาน PowerPoint (Master Update) ---
def create_summary_pptx(map_image_bytes, image_list, cable_type, route_distance, issue_kml_elements, template_bytes=None):
    ...
    p_ver = ver_box.text_frame.paragraphs[0]; p_ver.text = f"Ver.Update Data ปัจจุบัน {datetime.now().strftime('%d/%m/%Y')}"
    p_ver.font.size, p_ver.font.color.rgb = Pt(12), RGBColor(0, 0, 0)

    # ==========================================
    # --- หน้าที่ 2: สรุปแนวทางแก้ไขปัญหา (โครงสร้างใหม่) ---
    # ==========================================
    ...
    # ปัญหา สาเหตุและผลกระทบ (ขีดเส้นใต้)
...
st.markdown("""<style>
    ...
    .stButton>button { background: #2D5A27; color: white; border-radius: 14px; width: 100%; height: 50px; font-weight: 700; }
    ...
    with col_c2:
        if st.button("🚀 ดาวน์โหลดรายงาน PPTX"):
            try:
                ...
                pptx_d = create_summary_pptx(map_cap.getvalue(), st.session_state.export_data, cable_type, r_dist, k_els, t_bytes)
